...
## Статус
Фаза 0 + все mock-able фичи спеки (группа A) готовы: пауза записи, экран отказа микрофона (флаг `MOCK_MIC_DENIED`), jump по таймкоду, перегенерация саммари (мок реально меняет текст), rename/delete через меню «...», тост, мок-плеер, retry на карточке failed, сбой загрузки (флаг `MOCK_UPLOAD_FAILS`), ошибки логина по видам (флаг `MOCK_AUTH_FAILURE`), секции «Решения»/«Дальнейшие шаги» в саммари, empty state списка. Анимации прокачаны. Осознанные отличия от спеки задокументированы в README (генерация вручную, автоотправка в Inbox, обработка как состояние).
