...
app/                         экраны (expo-router, файловая маршрутизация)
  ...
  (app)/record.tsx           запись (таймер, облако точек, живая расшифровка)
  (app)/processing/[id].tsx  обработка (poll статусов → детали)
  (app)/recording/[id].tsx   детали: плеер + Транскрипт/Саммари + отправка
src/
...
| `recordingsApi.listRecordings` | `GET /api/mobile/recording/list` |
