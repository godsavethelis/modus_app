...
app/                         экраны (expo-router, файловая маршрутизация)
  ...
  (app)/photo/[id].tsx       лайтбокс: листание свайпом, «...» → переименовать/удалить
src/
...
**Фото в библиотеку (вне спеки, по запросу заказчика).** Фото — самостоятельный артефакт в общей ленте: чёрный «+» справа снизу раскрывает меню «Фото / Файл» (после скролла, когда герой-REC уходит с экрана, добавляется пункт «Запись»). «Фото» открывает шторку-галерею (паттерн Telegram: первый тайл — камера, мультивыбор кружочками, «Отправить N» создаёт N карточек). Камера — мок в нативном стиле iOS (стоковый кадр вместо видоискателя), после затвора — «Переснять / Отправить». Тап по готовой карточке — тёмный лайтбокс со статусом «В библиотеке». Топбар лайтбокса устроен как на экране записи: закрытие слева, «...» справа, иконки того же размера. Экран всегда тёмный независимо от темы, поэтому статус-бар переключается в светлые иконки (`useImmersiveScreen`; в вебе то же делает нарисованный статус-бар рамки телефона). Смахивание влево/вправо листает фото в порядке ленты, **пропуская аудиозаписи** — соседей отдаёт `recordingsApi.listPhotoIds()`; на краях фото пружинит. Позиция подписана в мете («2 из 4»), иначе о жесте не догадаться. Галерея и снимки — стоковые картинки с picsum.photos (нужна сеть); реальные камера/галерея — `TODO(recorder)` (expo-camera / expo-image-picker).
...
**Переименование и удаление быстрыми командами.** Заголовок записи можно править из трёх мест. В деталях записи и в просмотрщике фото — меню «...» в правом верхнем углу («Переименовать» / «Удалить»; у аудио ещё «Перегенерировать саммари»). На главной — свайп карточки влево, как в Apple Mail: открываются два тайла, чёрный «Имя» и красный «Удалить» (`src/components/SwipeableRow.tsx`). Одновременно открыта одна строка; тап по открытой карточке закрывает её, а не открывает запись. Протянул больше половины ширины — красный тайл растягивается на всю строку и на отпускании сразу спрашивает подтверждение («полный свайп» из Mail). Удаление всегда идёт через диалог: восстановления нет, корзины нет. Подтверждённая карточка схлопывается по высоте сразу, запрос уходит фоном. Жест скрытый, поэтому при первом открытии списка верхняя карточка один раз приоткрывается и возвращается — подсказка без текста. Диалоги вынесены в `RenameDialog` / `ConfirmDialog` с вариантом `tone="dark"` для лайтбокса фото. Жест сделан на `PanResponder` + `Animated`: gesture-handler и reanimated в проект намеренно не добавлялись. Тайлы смонтированы только пока строку тянут — в покое карточка накрывает их ровно, но веб-рамка масштабируется, и края округляются по-разному, из-под карточки лезла цветная кромка. Сдвиг анимируется без нативного драйвера: то же значение интерполируется в ширину красного тайла, а `width` нативному драйверу недоступен — при смешении `Animated.sequence` обрывалась и строка застревала приоткрытой.
